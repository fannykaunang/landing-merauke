...
export async function generatePageMetadata(
  ...
  try {
    ...
    const appName = settings?.alias_aplikasi || "merauke.go.id";
    ...
    // Domain dan URLs
    const domain = settings?.domain || "merauke.go.id";
    ...
    const keywords =
      ...
      (settings?.meta_keywords
        ...
        : ["merauke.go.id", "Portal", "Website", "Merauke"]);
    ...
    const title = options.title
      ? `${options.title} | merauke.go.id`
      : "merauke.go.id";
    const description =
      options.description || "Integrasi Laporan Kegiatan Online Digital ASN";

    return {
      metadataBase: new URL("https://merauke.go.id"),
      title,
      description,
      robots: {
...
export async function generateDocsMetadata(
  ...
  return generatePageMetadata({
    ...
    keywords: ["dokumentasi", "API", "developer", "merauke.go.id"],
  });
}
